...
\subsubsection{Penetapan Explainer TreeSHAP}

Sesuai landasan teori yang telah diuraikan pada Subbab~\ref{subsec:landasan-shap}, metode interpretasi yang digunakan pada penelitian ini adalah TreeSHAP (\emph{TreeExplainer}). Penetapan ini mengikuti karakteristik model prediksi akhir, yaitu XGBoost yang tergolong \emph{tree ensemble}. Sebagaimana dikembangkan oleh \textcite{Lundberg2020TreeSHAP}, TreeSHAP dirancang khusus untuk model berbasis pohon sehingga mampu menghitung nilai Shapley secara eksak (\emph{exact Shapley values}) dalam waktu polinomial dengan memanfaatkan struktur pohon keputusan, alih-alih melalui aproksimasi berbeban komputasi tinggi seperti pada pendekatan SHAP yang bersifat \emph{model-agnostic}. Dengan demikian, atribusi fitur yang dihasilkan bersifat tepat dan konsisten dengan aksioma nilai Shapley \autocite{Lundberg2017Unified}, sekaligus efisien untuk dijalankan pada sistem. Implementasi TreeSHAP pada penelitian ini dilakukan melalui pustaka \texttt{shap} dengan memanggil \emph{TreeExplainer} atas objek model XGBoost terlatih, sehingga terdapat keterkaitan yang konsisten antara landasan teori, perancangan, dan implementasi modul interpretasi.
