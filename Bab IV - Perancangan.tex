...
\textit{Prompt Augmentation}: Pada tahap ini, sistem menggabungkan tiga elemen menjadi satu instruksi yang komprehensif, yaitu (1) kueri asli, (2) hasil analisis SHAP berupa faktor-faktor risiko yang berkontribusi terhadap prediksi, dan (3) \textit{retrieved context} yang berisi dokumen pedoman. Pendekatan ini merupakan bentuk \textit{in-context learning}, yaitu teknik yang mengarahkan model untuk menghasilkan respons berdasarkan konteks yang disediakan dalam \textit{prompt}.            \item \textit{Large Language Model (Generator)}: Terakhir, LLM memproses \textit{prompt} yang sudah diperkaya (\textit{augmented}) tersebut untuk menyusun narasi bahasa alami yang koheren, personal, dan tervalidasi oleh dokumen medis yang dilampirkan dalam konteks.
        \end{enumerate}
    \end{enumerate}
\end{enumerate}
